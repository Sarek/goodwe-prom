use std::{fmt::Display, net::{Ipv4Addr, UdpSocket}, time::Duration};
...
pub fn get_metrics(target: &str, ms: &mut MetricSet) -> Result<(), MetricsError> {
  ...
  println!("Created socket. Connecting to {}", target);
  ...
  let _ = match sock.recv(&mut buf) {
      Ok(size) => {
        match modbus::get_payload(&buf[0..size].to_vec()) {
            Ok(data) => {
              print_buffer(&data);
              ms.read_data(&data)
            },
